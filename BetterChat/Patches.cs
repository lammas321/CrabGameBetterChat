    ...
    internal static class Patches
    {
        ...
        internal static bool PreServerSendSendChatMessage(bool __runOriginal, ulong param_0, string param_1)
        {
            ...
            foreach (ulong clientId in LobbyManager.steamIdToUID.Keys)
                if (clientId != param_0 || !string.IsNullOrEmpty(SteamMatchmaking.GetLobbyMemberData(SteamManager.Instance.currentLobby, new(clientId), MyPluginInfo.PLUGIN_GUID)))
                    SteamPacketManager.SendPacket(new(clientId), packet, 8, SteamPacketManager_NetworkChannel.ToClient);
        ...
        [HarmonyPrefix]
        internal static bool PreChatboxAppendMessage(ulong param_1, string param_3)
            => param_1 != 0UL || param_3 != SteamManager.Instance.field_Private_String_0 || string.IsNullOrEmpty(SteamMatchmaking.GetLobbyData(SteamManager.Instance.currentLobby, MyPluginInfo.PLUGIN_GUID));
        ...
        // Tells hosts with the mod you have BetterChat
        ...
        internal static void PostSteamManagerLobbyEnter()
            => SteamMatchmaking.SetLobbyMemberData(SteamManager.Instance.currentLobby, MyPluginInfo.PLUGIN_GUID, MyPluginInfo.PLUGIN_VERSION);
    }
}